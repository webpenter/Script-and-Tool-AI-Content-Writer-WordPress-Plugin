...
class WebPenter_ABA_Cron
{
  const CRON_HOOK = 'webpenter_aba_cron_generate';
  ...
  public static function init()
  {
    ...
    add_action('admin_post_webpenter_aba_generate_now', array(__CLASS__, 'handle_manual_run'));
    add_action('update_option_webpenter_aba_settings', array(__CLASS__, 'reschedule_event'));
  }
  ...
  public static function add_custom_schedules($schedules)
  {
    ...
    $schedules['aba_custom_interval'] = array(
  ...
  public static function process_cron()
  {
    $settings = WebPenter_ABA_Settings::get_settings();
    if ($settings['automation_status'] === 'enabled') {
      WebPenter_ABA_Generator::generate_batch();
    }
  }
  ...
  public static function handle_manual_run()
  {
    ...
    check_admin_referer('webpenter_aba_generate_now');

    $initial_errors = count(WebPenter_ABA_Settings::get_recent_errors());

    WebPenter_ABA_Generator::generate_batch();
    
    $final_errors = count(WebPenter_ABA_Settings::get_recent_errors());

    // Check if new errors occurred
    if ($final_errors > $initial_errors) {
        wp_safe_redirect(add_query_arg(
          array('page' => 'ai-blog-automator-settings', 'generation_result' => 'error', 'generation_message' => urlencode('Batch execution finished, but there were errors. Check Logs.')),
          admin_url('admin.php')
        ));
    } else {
        wp_safe_redirect(add_query_arg(
          array('page' => 'ai-blog-automator-settings', 'generation_result' => 'success', 'generation_message' => urlencode('Batch execution completed successfully!')),
          admin_url('admin.php')
        ));
    }
    exit;
