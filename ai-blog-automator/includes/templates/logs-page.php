...
$recent_errors = Bluteem_ABA_Settings::get_recent_errors();
...
<div class="wrap">
  ...
  if (
    ...
    && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'bluteem_aba_errors_feedback')
  ): ?>
      ...
      <?php if ($error_count > 0): ?>
          ...
          <?php foreach (array_reverse($recent_errors) as $error): ?>
            <div class="ai-blog-automator-log-entry">
              <p class="ai-blog-automator-log-time">
                <strong><?php echo esc_html(get_date_from_gmt($error['timestamp'], 'F j, Y g:i a')); ?></strong>
              </p>
        ...
        <div class="ai-blog-automator-logs-actions">
          <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            <input type="hidden" name="action" value="bluteem_aba_clear_errors">
            <?php wp_nonce_field('bluteem_aba_clear_errors'); ?>
